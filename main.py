...
class Student:
    ...
    def try_to_borrow(self, roomid, date_s, tstart, tend, groupname, topic, detail):
        success = '预约成功'
    ...
    def get_id_pass(self):
        ...
        if len(result) == 0:
            LOGGER.warning('该成员无已借阅讨论室\t人员id: %d', self.memberid)
        return result
    ...
    def apply(self, applicationid, password, roomname):
        ...
        if success in result.text:
            ...
            LOGGER.warning('申请加入失败\t人员id: %d\troomname:%s', self.memberid, roomname)
...
"""
VISIBLE_ROOMIDS = [1, 2, 3, 4, 5, 7, 8, 9, 11, 12, 15, 16, 19, 20, 21, 22, 23, 24, 25, 26]
GROUP_NAMES = ['自然语言处理小组', '图像识别小组', '音乐鉴赏讨论组', '数学分析交流', '线性代数讨论组', '自然语言处理小组']
TOPIC_NAMES = ['自然语言处理+词性标注', '图像识别+损失函数讨论', '音乐鉴赏讨论+巴洛克器乐', '数学分析交流+二重积分', '线性代数讨论+矩阵求导', '自然语言处理+语义角色标注']
DETAILS = ['词性标注实验结果分析（SVM和CRF）', '图像识别（由y和y_predict如何定义loss function较好）', '独奏协奏曲《春》的分析', '二重积分的技巧讨论', '矩阵求导+backpropagation', '自然语言处理小组+srl论文交流']


def borrow(apply_date,start,end,room,topic = None):
    """
        配置借图书馆的策略，理想情况下有四个参数，日期，开始时间，结束时间，时间是否可分割
    """
    # tomorrow， 格式为isoformat：2018-03-11
    if apply_date in [0,1,2]:
    ...
    groupname = GROUP_NAMES[idx]
    topic = TOPIC_NAMES[idx]
    detail = DETAILS[idx]
    leader = Student(0)
    # return True or False
    leader.try_to_borrow(roomid, date_s, tstart, tend, groupname, topic, detail)
...
def apply(applicationid, password, roomname):
    member0 = Student(0)
    member0.apply(applicationid, password, roomname)
    member1 = Student(1)
